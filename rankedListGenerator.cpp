...
{ // Returns false if left is better than right, true otherwise. Better quality < Worse quality.
    ...
    while (true)
    {
        ...
        cout << "Which item is better:" << endl
             << "1. " << left << endl
             << "2. " << right << endl;
        cin >> answer;
        cout << endl
...
{ // iterates through vector, (sub-vector) seperates as greater than/less than pivot.
    ...
    while (i <= j)
    {
        while (compare(pivotValue, items[i]))
        { // pivotValue > items[i]
            i++;
        }
        while (compare(items[j], pivotValue))
        { // items[j] > pivotValue
            j--;
        }
        if (i <= j)
...
{ // more spacially complex but perhaps faster.
